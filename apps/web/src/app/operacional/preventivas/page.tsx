...
import PreventivesManager from "@/components/operational-preventives/preventives-manager";
...
export default async function Page() {
  ...
  if (!user.permissoes?.includes("OPERACIONAL.PREVENTIVAS.VISUALIZAR"))
    redirect("/dashboard");
    ...
    <AppShell userName={user.nome} userEmail={user.email}>
      <PreventivesManager
        canManage={user.permissoes?.includes(
          "OPERACIONAL.PREVENTIVAS.GERENCIAR",
        )}
      />
    </AppShell>
  );
